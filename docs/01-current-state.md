...
| **Online / multiplayer** | Lobbies work; server-browser browse+join **field-confirmed on non-VAC** (B5, 2026-06-05); match-filter forwarding (B5.1) implemented, re-test pending; official = VAC (B7) | Online plumbing works: the engine reaches online mode (`BLoggedOn=1`, `GetConnectedUniverse=1`), and once the transient rate-limit cleared, **`CreateLobby` → `result=1` and lobby joins → `resp=1` (Success)** — local/online server creation works (2026-06-04). **B5 (2026-06-05, field-confirmed):** the dedicated-server browser previously stayed empty because the helper dropped the `ServerResponded` events real Steam delivers. The helper now **queues + forwards** them (`0xFFFFFFFD` drain envelope) and the bridge **re-dispatches** into the game's `ISteamMatchmakingServerListResponse` via a new `thiscall_run2`, plus a `gameserveritem_t` pack(4)→pack(8) fix in `GetServerDetails`. A live run re-dispatched 1600+ rows, the engine called `GetServerDetails` (never did before), and a **non-VAC dedicated join started a game**. **B5.1 (2026-06-05):** the game's match filters (`gamedir`/`gametype`/…) — previously dropped, leaving the browser unfiltered — are now serialized and forwarded to real Steam, so a campaign browse should list only campaign servers (re-test pending). **Official/VAC servers fail = B7 gate** (expected). See [Phase 3](08-roadmap.md#phase-3--online-multiplayer-join-official-steam-games). |
